...
public class Camera
{
	...
	/* Additional methods added to TAGE */

	// Turns camera around its V-axis
	public void yaw(float rotAmount){
		Vector3f rightVector, upVector, fwdVector;
		rightVector = getU(); 
  		upVector = getV(); 
  		fwdVector = getN(); 
		rightVector.rotateAxis(rotAmount, upVector.x(), upVector.y(), upVector.z()); 
		fwdVector.rotateAxis(rotAmount, upVector.x(), upVector.y(), upVector.z()); 
		setU(rightVector);
		setN(fwdVector);

	}

	// Rotates camera around its U-axis
	public void pitch(float rotAmount){
		Vector3f rightVector, upVector, fwdVector;
		rightVector = getU(); 
  		upVector = getV(); 
  		fwdVector = getN(); 
		fwdVector.rotateAxis(rotAmount, rightVector.x(), rightVector.y(), rightVector.z());
		upVector.rotateAxis(rotAmount, rightVector.x(), rightVector.y(), rightVector.z());
		setN(fwdVector);
		setV(upVector);
